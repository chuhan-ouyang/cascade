...
def invoke_directly(mnt_dir, name, options):
    ...
    if name == "cascade_fuse_client_hl":
...
def tst_read_content(mnt_dir, run_id=""):
...
# Test 4. check the write content of files
def tst_write_content(mnt_dir, run_id=""):
    # 1. subgroup content test
    subgroup_type = "/VolatileCascadeStoreWithStringKey/"
    for i in range(1, 2):
        sg_mnt_key = (
...
def cascade_fuse_mount(mnt_dir):
    ...
    cmd_input = base_cmdline + [pjoin(basename, "cascade_fuse_client_hl"), mnt_dir, "-f"]
